...
atRisk <- subset(
                dataTest,
                (dataTest$YearsAtCompany+1) > dataTest$PredictionYears.lm02
                )
...
```{r}
library(ROCR)
pred5 = prediction(dataTest$ProbabilityAttrition05, dataTest$Attrition)
pred6 = prediction(dataTest$ProbabilityAttrition06, dataTest$Attrition)

pred8 = prediction(dataTest$PredRF.probability, dataTest$Attrition)
perf = performance(pred5, "acc")
plot(perf)

perf_cost = performance(pred5, "cost")

plot(perf_cost)
roc = performance(pred5,"tpr","fpr")
roc6 = performance(pred6,"tpr","fpr")
#roc7 = performance(pred6,"tpr","fpr")
roc8 = performance(pred8,"tpr","fpr")
plot(roc, colorize = T, lwd = 2)
plot(roc6, colorize = F, lwd = 2, add = TRUE)
plot(roc8, col="yellow", lwd = 2, add = TRUE)
abline(a = 0, b = 1)
```
